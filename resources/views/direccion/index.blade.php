...
<h1 class="text-center">Direcciones</h1>
    <a href="direccions/create" class="btn btn-dark mb-2">Crear</a>
    ...
    <table id="table" class="table table-light table-striped mt-4">
        ...
        @foreach($direccions as $direccion)
            <tr>
                ...
                <td>{{$direccion->sigla}}</td>
                <td>{{$direccion->secretaria->sigla}}</td>
                <td>
                   <form action="{{ route ('direccions.destroy',$direccion->id)}}" method="POST">
                        @csrf
                        @method('DELETE')
                        <a href="/direccions/{{$direccion->id}}/edit" class="btn btn-info">Editar</a>
